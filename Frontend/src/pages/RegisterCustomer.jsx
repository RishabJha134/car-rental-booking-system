...
export default function RegisterCustomer() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true)
    try {
      const res = await api.post('/auth/register-customer', { name, email, password })
  ...
  return (
    <div className="mx-auto max-w-md rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl shadow-black/30 backdrop-blur">
      ...
      <p className="mt-2 text-sm text-slate-300">Register to browse cars and make bookings.</p>
      ...
      <p className="mt-5 text-sm text-slate-400">
        Already have an account? <Link to="/login" className="text-cyan-300 hover:text-cyan-200">Log in</Link>
      </p>
    </div>
  )
}
